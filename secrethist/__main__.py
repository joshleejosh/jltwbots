...
def find_params(api, trend=None, woeid=None):
    ...
    plog('%d/%d: %s'%(len(woes), c, '; '.join((w['name'] for w in woes.values()))))
    ...
    for wid,w in woes.iteritems():
        ...
        plog('%s (%d): %d trends'%(w['name'], w['woeid'], len(a)))
        woeTrends[w['woeid']] = a
        for t in a:
            trendCounts[t] += 1
    ...
    plog('Filter out %d conflicting trends'%len(conflicts))
    todel = []
    ...
    plog(u'%s/%d: %d/%d: %s'%(woe['name'], woeid, len(trends), c, '; '.join(trends)))
...
def main(args):
    ...
    if woe:
        woeid = woe['woeid']
        plog('=======> %s %d: %s'%(woe['name'], woeid, trend))
    if not woe or not woeid or not trend:
        plog('FAILURE: Couldn\'t find parameter [%s] [%d] [%s]'%(woe, woeid, trend))
        return
    ...
    if len(tweets) > 0:
        ...
        plog('FAILURE: No tweets for trend [%s]'%trend)
