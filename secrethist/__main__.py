...
def find_params(api, trend=None, woeid=None):
    ...
    jltw.log('%d/%d: %s'%(len(woes), c, '; '.join((w['name'] for w in woes.values()))))
    ...
    for wid,w in woes.iteritems():
        ...
        jltw.log('%s (%d): %d trends'%(w['name'], w['woeid'], len(a)))
        woeTrends[w['woeid']] = a
        for t in a:
            trendCounts[t] += 1
    ...
    jltw.log('Filter out %d conflicting trends'%len(conflicts))
    todel = []
    ...
    if not woeTrends:
        return woe, trend
    for w,a in woeTrends.iteritems():
        jltw.log('%d: %d local trends'%(w, len(a)))
    ...
    jltw.log(u'%s/%d: %d/%d: %s'%(woe['name'], woeid, len(trends), c, '; '.join(trends)))
...
def main(args):
    ...
    if woe:
        woeid = woe['woeid']
        jltw.log('=======> %s %d: %s'%(woe['name'], woeid, trend))
    if not woe or not woeid or not trend:
        jltw.log('FAILURE: Couldn\'t find parameter [%s] [%d] [%s]'%(woe, woeid, trend))
        return
    ...
    if len(tweets) > 0:
        ...
        jltw.log('FAILURE: No tweets for trend [%s]'%trend)
